...
q3.rdd.map(lambda i: ','.join(str(j) for j in i)) \
	.saveAsTextFile('junqich_si618_project_q1')

q3_1 = sqlContext.sql("SELECT country, year, avg_reproduction_rate FROM covid_R_vacc")
q3_1.rdd.map(lambda i: ','.join(str(j) for j in i)) \
	.saveAsTextFile('junqich_si618_project_q1-1')

q3_2 = sqlContext.sql("SELECT * FROM covid_R_vacc WHERE year = 2021 ORDER BY tot_vacc_per_million DESC")
...
q5.rdd.map(lambda i: ','.join(str(j) for j in i)) \
	.saveAsTextFile('junqich_si618_project_q2')

q5_1 = sqlContext.sql("SELECT country, year, avg_stringency_index FROM covid_S_vacc")
q5_1.rdd.map(lambda i: ','.join(str(j) for j in i)) \
	.saveAsTextFile('junqich_si618_project_q2-1')

q5_2 = sqlContext.sql("SELECT * FROM covid_S_vacc WHERE year = 2021 ORDER BY tot_vacc_per_million DESC")
